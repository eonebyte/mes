export default async (server, opts) => {
    server.post('/hold', async (request, reply) => {
        try {
            ...
            const result = await server.plan.doHold(request, server, parseInt(planId), parseInt(resourceId), status, parseInt(productionId), parseInt(togoQty), parseInt(outputQty));
